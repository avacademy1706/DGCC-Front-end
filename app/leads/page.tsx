...
const avatarGradient = (name: string) => {
  const g = ["from-indigo-500 to-purple-600","from-pink-500 to-rose-600","from-amber-500 to-orange-600","from-emerald-500 to-teal-600","from-cyan-500 to-blue-600","from-violet-500 to-indigo-600"];
  return g[(name.charCodeAt(0) || 0) % g.length];
};
...
const META_SYSTEM_KEYS = new Set([
  "lead_id","form_id","form_name","created_time","ad_id","ad_name","adset_id",
  "adset_name","campaign_id","platform","is_organic","full_name","name",
  "first_name","last_name","email","email_address","phone_number","phone",
  "mobile","city","state","clientId",
]);
...
function StatCard({ icon: Icon, value, label, sub, accent = "indigo" }: {
  ...
  return (
    <div className="p-3 sm:p-4 md:p-5 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-[#0b1220]">
      <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center mb-2 sm:mb-3 ${ac[accent]}`}>
        <Icon size={14} />
      </div>
      <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white leading-none">{value}</p>
      <p className="text-[11px] sm:text-xs text-gray-500 dark:text-slate-400 mt-1 leading-tight">{label}</p>
      {sub && <p className="text-[11px] sm:text-xs text-emerald-500 mt-0.5">{sub}</p>}
    </div>
  );
}

// ─── Lead Drawer ──────────────────────────────────────────────────────────────
function LeadDrawer({ lead, onClose, clientName }: { lead: Lead; onClose: () => void; clientName?: string }) {
...
// ─── Mobile Lead Card ─────────────────────────────────────────────────────────
function MobileLeadCard({ lead, onClick, clientName }: { lead: Lead; onClick: () => void; clientName?: string }) {
  const name = getName(lead);
  return (
    <div onClick={onClick} className="flex items-start gap-3 p-4 border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 dark:hover:bg-slate-800/50 active:bg-indigo-50/50 cursor-pointer transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="text-[10px] text-gray-400 dark:text-slate-500 shrink-0">{timeAgo(lead.created_time)}</span>
        </div>
        {getPhone(lead) !== "—" && (
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5 flex items-center gap-1">
            <Phone size={10} /> {getPhone(lead)}
        ...
        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
          ...
          <span className="text-[10px] text-gray-400 dark:text-slate-500 truncate max-w-[150px]">{lead.form_name}</span>
          {lead.platform && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full border font-medium capitalize ${platformBadge[lead.platform] || "bg-gray-100 dark:bg-slate-800 text-gray-500 border-gray-200 dark:border-slate-700"}`}>
              {lead.platform}
            </span>
          )}
        </div>
      </div>
      <ChevronRight size={14} className="text-gray-300 dark:text-slate-600 shrink-0 mt-2" />
    </div>
  );
}
  ...
  return (
    <div onClick={onClick} className="bg-white dark:bg-[#0b1220] border border-gray-200 dark:border-slate-800 rounded-xl p-3 cursor-pointer hover:border-indigo-400 dark:hover:border-indigo-500 hover:shadow-md transition-all group">
      <div className="flex items-center gap-2 mb-2">
        ...
        <ChevronRight size={11} className="text-gray-300 dark:text-slate-600 shrink-0 group-hover:text-indigo-400 transition-colors" />
      </div>
      {getPhone(lead) !== "—" && (
        <p className="text-[11px] text-gray-500 dark:text-slate-400 truncate mb-1">📞 {getPhone(lead)}</p>
      )}
      {clientName && (
      ...
      <p className="text-[10px] text-gray-400 dark:text-slate-500 truncate mb-2">{lead.form_name}</p>
      <div className="flex items-center justify-between">
        {lead.platform ? (
          <span className={`text-[10px] px-1.5 py-0.5 rounded-full border font-medium capitalize ${platformBadge[lead.platform] || "bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400 border-gray-200 dark:border-slate-700"}`}>
            {lead.platform}
          </span>
        ) : <span />}
        <span className="text-[10px] text-gray-400 dark:text-slate-600">{timeAgo(lead.created_time)}</span>
      </div>
    </div>
  );
}
...
const PIPELINE_STAGES = [
  { key: "new",       title: "New",       color: "bg-blue-500/90  border border-blue-500/20"    },
  { key: "working",   title: "Working",   color: "bg-amber-500/90  border border-amber-500/20"  },
  { key: "qualified", title: "Qualified", color: "bg-indigo-500/90  border border-indigo-500/20"},
  { key: "won",       title: "Won",       color: "bg-emerald-500/90  border border-emerald-500/20"},
  { key: "lost",      title: "Lost",      color: "bg-red-500/90  border border-red-500/20"        },
  { key: "nurture",   title: "Nurture",   color: "bg-purple-500/90  border border-purple-500/20"},
];
...
export default function LeadManagement() {
  ...
  const [filterForm,      setFilterForm]      = useState("");
  const [filterPlatform,  setFilterPlatform]  = useState("");
  const [filterClient,    setFilterClient]    = useState("");  // ← NEW
  const [selectedLead,    setSelectedLead]    = useState<Lead | null>(null);
  const [page,            setPage]            = useState(1);
  const [showFilters,     setShowFilters]     = useState(false);

  // ── Build leads URL with optional clientId param ───────────────────────────
  const leadsParams = new URLSearchParams();
  if (filterClient) leadsParams.set("clientId", filterClient);
  const leadsUrl = `/leads${leadsParams.toString() ? `?${leadsParams}` : ""}`;
  ...
  // ── Stats ──────────────────────────────────────────────────────────────────
  const today      = new Date().toDateString();
  const todayCount = leads.filter(l => new Date(l.created_time).toDateString() === today).length;

  const uniqueForms = useMemo(
    () => [...new Map(leads.map(l => [l.form_id, { id: l.form_id, name: l.form_name }])).values()],
    [leads],
  );
  const uniquePlatforms = useMemo(
    () => [...new Set(leads.map(l => l.platform).filter(Boolean))] as string[],
  ...
  // ── Client-side filtering (search + form + platform) ──────────────────────
  // Note: client filter is server-side (passed in URL), others are client-side
  const filtered = useMemo(() => {
    const q = search.toLowerCase();
    return leads.filter(l => {
      const matchSearch = !q
  ...
  const clearAllFilters = () => {
    setSearch(""); setFilterForm(""); setFilterPlatform("");
    setFilterClient(""); setPage(1); setShowFilters(false);
  };

  // Selected client label for display
  const selectedClientName = filterClient ? (clientNameMap[filterClient] || "Client") : "";
  ...
  return (
    <div className="flex flex-col h-screen w-full min-h-0 max-h-screen bg-gray-50 dark:bg-[#020817] text-gray-900 dark:text-white overflow-x-hidden">
      ...
      <div className="shrink-0 px-4 pt-4 sm:pt-5 sm:px-6 md:px-8 space-y-3 sm:space-y-4 pb-0 border-b border-gray-200 dark:border-slate-800">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5 flex items-center gap-2 flex-wrap">
              ...
              {isLoading ? (
                ...
                <span className="text-emerald-500 font-medium">{data?.total ?? 0} leads</span>
              )}
        ...
        {/* Stats */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          <StatCard icon={TrendingUp} accent="indigo" value={isLoading ? "—" : todayCount}          label="Today's Leads" sub="Live from Meta" />
          <StatCard icon={BarChart3}  accent="blue"   value={isLoading ? "—" : (data?.total ?? 0)}  label="Total Leads" />
          <StatCard icon={Megaphone}  accent="amber"  value={isLoading ? "—" : uniqueForms.length}  label="Active Forms" />
          <StatCard icon={Star}       accent="emerald" value={isLoading ? "—" : (uniquePlatforms.length || "—")} label="Platforms" />
        </div>

        {/* Tabs */}
        <div className="flex gap-4 sm:gap-6 text-sm -mb-px">
          {[{ key: "table", label: "All Leads" }, { key: "pipeline", label: "Pipeline" }].map(tab => (
        ...
        <div className="flex-1 min-h-0 flex flex-col px-4 py-3 sm:px-6 sm:py-4 md:px-8 gap-3 overflow-hidden" style={{ minHeight: 0 }}>
          ...
          <div className="hidden sm:flex flex-row gap-2 shrink-0">
            ...
            <div className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-[#0b1220] flex-1">
              ...
              {search && (
                <button onClick={() => { setSearch(""); setPage(1); }}>
                  <X size={13} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300" />
                </button>
              )}
            </div>

            {/* ── Client filter — always shown if clients exist ── */}
            {clients.length > 0 && (
              <select value={filterClient} onChange={e => { setFilterClient(e.target.value); setPage(1); }}
                className="px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-[#0b1220] text-sm text-gray-700 dark:text-gray-200 outline-none max-w-[180px]">
                <option value="">All Clients</option>
                {clients.map(c => (
                  <option key={c._id} value={c._id}>{c.profile?.companyName || "Unnamed"}</option>
                ))}
              </select>
            )}
            ...
            {uniqueForms.length > 1 && (
              <select value={filterForm} onChange={e => { setFilterForm(e.target.value); setPage(1); }}
                className="px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-[#0b1220] text-sm text-gray-700 dark:text-gray-200 outline-none max-w-[180px]">
                <option value="">All Forms</option>
                {uniqueForms.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
              </select>
            )}
            ...
            {uniquePlatforms.length > 1 && (
              <select value={filterPlatform} onChange={e => { setFilterPlatform(e.target.value); setPage(1); }}
                className="px-3 py-2 rounded-lg border border-gray-200 dark:border-slate-700 bg-white dark:bg-[#0b1220] text-sm text-gray-700 dark:text-gray-200 outline-none capitalize">
                <option value="">All Platforms</option>
                {uniquePlatforms.map(p => <option key={p} value={p} className="capitalize">{p}</option>)}
              </select>
            )}
          ...
          {isError && (
            <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-6">
              ...
              <p className="font-semibold text-red-400 text-sm">Could not load leads</p>
              <button onClick={() => refetch()} className="px-4 py-2 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">Retry</button>
            </div>
          )}
          ...
          {!isError && (
            <div className="md:hidden flex-1 min-h-0 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-[#0b1220] overflow-y-auto overscroll-contain"
              style={{ scrollbarWidth: "thin", scrollbarColor: "rgb(99 102 241 / 0.3) transparent" }}>
              ...
              {!isLoading && paginated.map(lead => (
                <MobileLeadCard key={lead.lead_id} lead={lead} clientName={getClientName(lead)} onClick={() => setSelectedLead(lead)} />
              ))}
            </div>
          )}
            ...
            <div className="hidden md:flex flex-col flex-1 min-h-0 rounded-xl border border-gray-200 dark:border-slate-800 bg-white dark:bg-[#0b1220] overflow-hidden"
              ...
              <div className="w-full h-full overflow-auto">
                <table className="w-full text-sm table-fixed">
                  <thead className="sticky top-0 z-10 bg-gray-50/90 dark:bg-slate-900/90 backdrop-blur-sm border-b border-gray-200 dark:border-slate-800">
                    <tr>
                      <th className="px-3 py-3 w-[220px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Lead</th>
                      <th className="px-3 py-3 w-[150px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Client</th>
                      <th className="px-3 py-3 w-[200px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Email</th>
                      <th className="px-3 py-3 w-[140px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Phone</th>
                      <th className="px-3 py-3 w-[180px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Form / Ad</th>
                      <th className="px-3 py-3 w-[110px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Platform</th>
                      <th className="px-3 py-3 w-[110px] text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">Received</th>
                      <th className="px-3 py-3 w-[40px]" />
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100 dark:divide-slate-800/70">
                        ...
                        <tr key={lead.lead_id} onClick={() => setSelectedLead(lead)}
                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-2 max-w-[220px]">
                              ...
                              <div className="min-w-0">
                                <p className="font-semibold text-gray-900 dark:text-white truncate">{name}</p>
                                {getCity(lead) !== "—" && <p className="text-[11px] text-gray-400 truncate">{getCity(lead)}</p>}
                              </div>
                            </div>
                          </td>
                          {/* Client */}
                          <td className="px-3 py-3">
                            {clientName ? (
                              <span className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-medium truncate max-w-[130px]">
                                <Building2 size={9} className="shrink-0" /> {clientName}
                              </span>
                            ) : (
                          ...
                          {/* Email */}
                          <td className="px-3 py-3">
                            <p className="truncate text-gray-600 dark:text-slate-400 text-xs">{getEmail(lead)}</p>
                          </td>
                          {/* Phone */}
                          <td className="px-3 py-3">
                            <p className="truncate text-gray-600 dark:text-slate-400 text-xs">{getPhone(lead)}</p>
                          </td>
                          {/* Form / Ad */}
                          <td className="px-3 py-3">
                            <p className="text-xs font-medium truncate">{lead.form_name}</p>
                            {lead.ad_name && <p className="text-[11px] text-gray-400 truncate">{lead.ad_name}</p>}
                          </td>
                          {/* Platform */}
                          <td className="px-3 py-3">
                            {lead.platform ? (
                          ...
                          {/* Received */}
                          <td className="px-3 py-3">
                            <p className="text-xs font-medium">{timeAgo(lead.created_time)}</p>
                            <p className="text-[11px] text-gray-400">{formatDate(lead.created_time)}</p>
                          </td>
                          {/* Action */}
                          <td className="px-3 py-3">
          ...
          {!isLoading && !isError && totalPages > 1 && (
            <div className="flex items-center justify-between shrink-0 pt-1">
              <p className="text-xs text-gray-500 dark:text-slate-400">
                ...
                <span className="hidden sm:inline"> of </span>
                <span className="sm:hidden">/</span>
                <span className="font-medium text-gray-700 dark:text-slate-300"> {filtered.length}</span>
              </p>
              <div className="flex gap-1">
                <button onClick={() => setPage(p => Math.max(1, p-1))} disabled={page===1}
                  className="px-2.5 sm:px-3 py-1.5 text-xs rounded-lg border border-gray-200 dark:border-slate-700 disabled:opacity-40 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">←</button>
                {Array.from({ length: Math.min(3, totalPages) }, (_, i) => {
                  const p = totalPages<=3 ? i+1 : page<=2 ? i+1 : page>=totalPages-1 ? totalPages-2+i : page-1+i;
                  return (
                    <button key={p} onClick={() => setPage(p)}
                      className={`px-2.5 sm:px-3 py-1.5 text-xs rounded-lg border transition-colors ${p===page ? "bg-indigo-600 border-indigo-600 text-white" : "border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-800"}`}>
                      {p}
                    </button>
                  );
                })}
     ...
     {activeTab === "pipeline" && (
        <div className="flex flex-col min-h-0 flex-1 px-4 py-3 sm:px-6 sm:py-4 md:px-8 gap-3 overflow-hidden">
            ...
            <div className="shrink-0 flex items-center gap-2 flex-wrap">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${
                  !filterClient
                    ? "bg-indigo-600 border-indigo-600 text-white"
                    : "border-gray-200 dark:border-slate-700 bg-white dark:bg-[#0b1220] text-gray-600 dark:text-gray-300 hover:border-indigo-400 dark:hover:border-indigo-500"
                }`}
              >
                All Clients
                {!filterClient && (
                  <span className="bg-white/25 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold">{leads.length}</span>
                )}
              </button>
              ...
              {clients.map(c => {
                ...
                const count    = leads.filter(l => l.clientId === c._id).length;
                return (
                  <button
                    key={c._id}
                  ...
                  >
                    ...
                    {cName}
                    {count > 0 && (
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${isActive ? "bg-white/25 text-white" : "bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400"}`}>
                        {count}
                      </span>
                    )}
                  </button>
              ...
              {filterClient && (
                <button onClick={() => setFilterClient("")} className="flex items-center gap-1 px-2.5 py-1.5 text-xs rounded-full border border-red-200 dark:border-red-900/50 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
                  <X size={10} /> Clear
                </button>
              )}
            </div>
          ...
          {/* Active client label */}
          {selectedClientName && (
            <div className="shrink-0 flex items-center gap-2 text-xs text-indigo-400">
              <Building2 size={11} />
              <span>Showing leads for <span className="font-semibold">{selectedClientName}</span></span>
            </div>
          )}
          ...
          {isError ? (
            ...
            <div className="h-full w-full overflow-x-auto overflow-y-hidden"
              ...
              <div className="h-full flex gap-2.5 sm:gap-3 md:gap-4 w-max">
                {stageLeads.map(col => (
                  <div key={col.key} className="w-[170px] sm:w-[190px] flex-shrink-0 flex flex-col gap-2 overflow-y-auto overscroll-contain pb-4">
                    <div className={`sticky top-0 z-10 flex justify-between items-center px-2.5 sm:px-3 py-2 rounded-xl text-xs font-semibold ${col.color}`}>
                      <span className="truncate">{col.title}</span>
                      <span className="bg-white/20 dark:bg-black/20 px-2 py-0.5 rounded-full font-bold ml-1 shrink-0">
                        {isLoading ? "—" : col.leads.length}
                      </span>
                    ...
                    {!isLoading && col.leads.map(lead => (
                      <PipelineCard key={lead.lead_id} lead={lead} clientName={getClientName(lead)} onClick={() => setSelectedLead(lead)} />
                    ))}
                  </div>
                ))}
              </div>
